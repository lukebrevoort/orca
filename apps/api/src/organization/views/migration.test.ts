...
describe("BRE-313 Views migration", () => {
  test("stores definitions without membership and installs supporting query indexes", () => {
    ...
    try {
      ...
      const indexes = client.sqlite.query("SELECT name FROM sqlite_master WHERE type = 'index' AND name IN ('organization_views_workspace_position_idx','organization_thread_facet_values_lookup_idx','emails_thread_view_evidence_idx','threads_view_order_idx') ORDER BY name").all() as Array<{ name: string }>;
      assert.deepEqual(indexes.map((item) => item.name), ["emails_thread_view_evidence_idx", "organization_thread_facet_values_lookup_idx", "organization_views_workspace_position_idx", "threads_view_order_idx"]);
