...
export function createSqliteOrganizationViewsRepository(sqlite: Database): OrganizationViewsRepository {
  ...
  return {
    ...
    query({ scope, view, query }) {
      ...
      return organizationViewResultPageSchema.parse({ viewId: view.id, viewRevision: view.revision, accountIds, items, nextCursor: hasMore ? encodeCursor(items.at(-1)!, cursorFingerprint) : null, limit: query.limit });
    },
  };
}
