import { effectiveClassificationSql, viewPredicates, threadMatchPredicate, normalizedEmailSql, normalizedAddressSql } from "./thread-predicate.ts";
...
type ViewRow = {
  ...
  skip_inbox: number; definition: string; revision: number; created_at: number; updated_at: number;
};
...
function mapView(row: ViewRow): OrganizationView {
  return organizationViewSchema.parse({
    ...
    skipInbox: Boolean(row.skip_inbox), position: row.position, definition: JSON.parse(row.definition), revision: row.revision,
...
export function buildOrganizationViewPageKeyQuery(input: {
  ...
  const { conditions, params, accountIds } = threadMatchPredicate(scope, input);
  ...
  return {
    ...
    create({ workspaceId, viewId, request, boundRequest, plan, authorization, now }) {
      ...
      return authorizedMutation(sqlite, { workspaceId, boundRequest, plan, authorization, now, mutate: () => {
        ...
        sqlite.query("INSERT INTO organization_views (workspace_id,id,name,description,color,position,definition,revision,created_at,updated_at,skip_inbox) VALUES (?,?,?,?,?,?,?,?,?,?,?)")
          .run(workspaceId, viewId, request.name, request.description, request.color, temporaryPosition, JSON.stringify(request.definition), 1, timestamp, timestamp, Number(request.skipInbox));
    ...
    update({ workspaceId, viewId, request, boundRequest, plan, authorization, now }) {
      return authorizedMutation(sqlite, { workspaceId, boundRequest, plan, authorization, now, mutate: () => {
        ...
        const result = sqlite.query("UPDATE organization_views SET name=?,description=?,color=?,position=?,definition=?,skip_inbox=?,revision=revision+1,updated_at=? WHERE workspace_id=? AND id=? AND revision=? RETURNING id")
          .all(next.name, next.description, next.color, plan.orderedViewIds.indexOf(viewId), JSON.stringify(next.definition), Number(next.skipInbox), now.getTime(), workspaceId, viewId, request.expectedRevision);
        if (result.length !== 1) throw new OrganizationViewConflictError();
        return get(workspaceId, viewId)!;
      } });
    },
