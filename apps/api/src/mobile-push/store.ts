...
type DeviceRow = MobilePushDevice & { tokenEncrypted: string; tokenHash: string; eligibleAfterAt: number; watermarkSequence: number; watermarkCreatedAt: number; watermarkEmailId: string };
...
export function listDevices(sqlite: Database, userId: string, installationId?: string) {
  ...
  return rows.map(({ tokenEncrypted: _tokenEncrypted, tokenHash: _tokenHash, eligibleAfterAt: _eligibleAfterAt, watermarkSequence: _watermarkSequence, watermarkCreatedAt: _watermarkCreatedAt, watermarkEmailId: _watermarkEmailId, sessionId: _sessionId, ...device }) => device);
}
