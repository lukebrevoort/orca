...
export function registerMcpOAuthRoutes(
  ...
  app.get("/oauth/authorize", requireMcpFeature, requireAuth({ dbFactory }), async (c) => {
    ...
    try {
      ...
      if (!scopes) return redirectError("invalid_scope", "Request one or more supported read-only scopes");
      ...
      const scopeItems = scopes.map((scope) => `<li><strong>${escapeHtml(scope)}</strong><span>${scope === "mail:read" ? "Read bounded Orca mail and thread data." : "Read propagated agent event status."}</span></li>`).join("");
