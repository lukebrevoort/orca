...
import { OrganizationAuthorityProvider, OrganizationRecoveryBanner, useOrganizationAuthority } from "./organization-authority";
...
export function OrganizationLaneWorkspace({ demoMode = false, onWorkspaceMutation, refreshToken = 0 }: { demoMode?: boolean; onWorkspaceMutation?: () => void; refreshToken?: number }) {
  ...
  useEffect(() => {
    ...
    setConfiguration(authority.snapshot.laneConfiguration);
    setSelectedLaneId((current) => authority.snapshot!.laneConfiguration.lanes.some((lane) => lane.id === current && !lane.retiredAt) ? current : authority.snapshot!.laneConfiguration.lanes.find((lane) => !lane.retiredAt)?.id ?? null);
    setState("ready");
  ...
  async function apply(actions: OrganizationLaneAction[]) {
    if (!configuration || state === "saving" || !canMutate) return;
    ...
    try {
      if (demoMode) {
        ...
        const body = await authority.request("/v1/organization/apply", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: nextId(), idempotencyKey: `web:lanes:${nextId()}`, expectedWorkspaceRevision: configuration.workspaceRevision, actions }) }, { operation: "mutation", capability: "apply", hasReliableData: true });
  ...
  async function createLane() {
    if (!configuration || !newLaneName.trim() || !canMutate) return;
  ...
  function reorder(lane: Lane, direction: -1 | 1) {
    if (!configuration || !canMutate) return;
  ...
  return <section className="lane-workspace" aria-labelledby="lane-workspace-title" aria-busy={state === "loading" || state === "saving" || undefined}>
    ...
    {state === "loading" ? <div className="lane-loading" role="status">Getting Workspace Lanes…</div> : configuration ? <div className="lane-workspace-grid">
      ...
      <aside className="lane-policy-panel" aria-label="Selected Lane Policy">{selectedLane && selectedPolicy ? <>
        ...
        <button className="lane-retire" disabled={!canMutate || state === "saving" || selectedLane.id === configuration.fallbackLaneId} onClick={() => void apply([{ kind: "update_lane", laneId: selectedLane.id, retired: true, expectedRevision: selectedLane.revision }])} type="button">Retire Lane</button>
...
function ThreadLaneControlsContent({ accountId, threadId, demoMode = false }: { accountId: string; threadId: string; demoMode?: boolean }) {
  const authority = useOrganizationAuthority();
  const [configuration, setConfiguration] = useState<OrganizationLaneConfiguration | null>(demoMode ? organizationLaneConfigurationFixture : null);
  ...
  const canApply = demoMode || authority.state.canMutate && authority.allows.apply;
  ...
  useEffect(() => {
    ...
    void authority.request(`/v1/organization/query?accountId=${encodeURIComponent(accountId)}&threadId=${encodeURIComponent(threadId)}&attention=all&classification=all&limit=1`, { signal: controller.signal }, { operation: "read", capability: "query", hasReliableData: Boolean(placement) }).then((body) => {
  ...
  }, [accountId, authority.refreshToken, authority.snapshot, demoMode, threadId]);
  ...
  async function apply(action: OrganizationLaneAction) {
    if (!configuration || !placement || state === "saving" || !canApply) return;
    setState("saving"); setError(null);
    try {
      if (demoMode) {
        ...
        const body = await authority.request("/v1/organization/apply", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: nextId(), idempotencyKey: `web:thread-lane:${nextId()}`, expectedWorkspaceRevision: configuration.workspaceRevision, actions: [action] }) }, { operation: "mutation", capability: "apply", hasReliableData: true });
  ...
  async function correct() {
    if (!configuration || !placement || state === "saving" || !canCorrect) return;
    ...
    try {
      if (demoMode) {
        ...
        const result = orcaThreadCorrectionResponseSchema.parse(await authority.request(`/v1/organization/threads/${encodeURIComponent(threadId)}/correct`, {
          method: "POST",
        ...
        }, { operation: "mutation", capability: "correct", hasReliableData: true }));
        const refreshed = organizationQueryResponseSchema.parse(await authority.request(`/v1/organization/query?accountId=${encodeURIComponent(accountId)}&threadId=${encodeURIComponent(threadId)}&attention=all&classification=all&limit=1`, undefined, { operation: "read", capability: "query", hasReliableData: true }));
...
export function ThreadLaneControls(props: { accountId: string; threadId: string; demoMode?: boolean }) {
  return <OrganizationAuthorityProvider previewMode={props.demoMode}><ThreadLaneControlsContent {...props} /></OrganizationAuthorityProvider>;
}
