// THROWAWAY BRE-380: compare Search, Workbench, and Inbox drawer on the existing
...
const names = ["Search first", "Live workbench", "Beside your mail"];
...
export function EffortlessViewsPrototype() {
  ...
  const [entry, setEntry] = useState<"examples" | "search">("search");
  ...
  function group() {
    ...
    preview({ name: "Selected senders", source: "examples", definition: { subject: "", unread: false, senders: [...new Set(examples.map(m => m.address))] } });
  }
  function save() {
  ...
  const entryControls = <div className="ev-entry">
    <div className="ev-tabs" aria-label="Start a view">{(["search", "examples"] as const).map((e,i) => <button key={e} aria-pressed={entry === e} onClick={() => {setEntry(e);setNotice("");}}>{["Search mail", "Use selected mail"][i]}</button>)}</div>
    {entry === "examples" ? <><p className="ev-note">Select messages to create a view from their senders.</p>{variant !== 2 ? mailRows(mail.slice(0,4), true) : null}<button className="ev-primary" disabled={!selected.length} onClick={group}>Use these senders · {selected.length}</button></> : null}
    {entry === "search" ? <><label className="ev-field"><span>Search mail by subject</span><input value={query} onChange={e => setQuery(e.target.value)} /></label><p className="ev-note">{match({subject:query, senders:[], unread:false}).length} matching conversations · exact subject text</p>{variant !== 1 ? mailRows(match({subject:query,senders:[],unread:false})) : null}<button className="ev-primary" disabled={!query.trim()} onClick={() => preview({name:query[0]!.toUpperCase()+query.slice(1),source:"search",definition:{subject:query,senders:[],unread:false}})}>Preview this view</button></> : null}
  </div>;
  const editingControls = draft ? <div className="ev-edit">
    ...
    <div className="ev-refine"><button aria-pressed={draft.definition.unread} onClick={() => update({...draft.definition,unread:!draft.definition.unread})}>Unread only</button>{undo ? <button onClick={() => {setDraft(undo);setUndo(null);setCorrection(null);}}>Undo refinement</button> : null}</div>
    <details className="ev-details"><summary>Edit filters</summary><label className="ev-field"><span>Subject contains</span><input value={draft.definition.subject} onChange={e => update({...draft.definition,subject:e.target.value})} /></label><fieldset><legend>Limit to a sender</legend>{[...new Set(mail.map(m => m.address))].map(address => <button key={address} aria-pressed={draft.definition.senders.length === 1 && draft.definition.senders[0] === address} onClick={() => update({...draft.definition,senders:[address]})}>{mail.find(m=>m.address===address)!.sender}</button>)}</fieldset>{draft.definition.senders.length ? <button onClick={() => update({...draft.definition,senders:[]})}>Include all senders</button> : null}<p className="ev-note">These filters are the exact interpretation being previewed.</p></details>
  ...
  const resultPanel = <section className="ev-results" aria-label="Matching mail"><header><div><span className="ev-eyebrow">{draft ? "Preview before saving" : current ? "Your saved view" : "Your mail, your way"}</span><h3>{current ? `${results.length} ${results.length === 1 ? "conversation" : "conversations"}` : "See what belongs."}</h3></div>{current ? <span className="ev-count">{results.length.toString().padStart(2,"0")}</span> : null}</header>
    {correction && draft ? <div className="ev-correction"><strong>What should change?</strong><p>Keep the other matching senders and leave out {correction.sender}. New senders will also stay outside this view until you add them.</p><button disabled={!results.some(m=>m.address!==correction.address)} onClick={() => {update({...draft.definition,senders:[...new Set(results.filter(m=>m.address!==correction.address).map(m=>m.address))]});setCorrection(null);}}>Keep the other senders</button><button onClick={() => setCorrection(null)}>Keep current matches</button></div> : null}
    {current ? results.length ? mailRows(results) : <div className="ev-empty"><h3>No mail matches yet.</h3><p>Broaden the subject or senders. You can also save this view for future mail.</p></div> : entry === "search" ? mailRows(match({subject:query,senders:[],unread:false})) : <div className="ev-empty"><span className="ev-big-wave" aria-hidden="true">≈</span><p>Start with a search or select<br/>the senders you want to follow.</p></div>}
  </section>;
  return <section className={`ev-prototype ev-variant-${variant}`} aria-label="Effortless views prototype">
    <div className="ev-lab"><span>INTERACTION STUDY · BRE-380</span><span>Sample mail · no AI · memory only</span></div>
    <header className="ev-heading"><div><span className="ev-eyebrow">Organization / Your views</span><h1>{variant === 0 ? "Find it. Keep it together." : variant === 1 ? "A view that fits you." : "Organize from here."}</h1></div><button onClick={() => {setDraft(null);setActive(null);setEditing(null);setUndo(null);setNotice("");}}>New view +</button></header>
    ...
    <div className="ev-study-tools"><button onClick={() => setInspect(!inspect)} aria-expanded={inspect}>Inspect prototype state</button><button onClick={() => setGuide(!guide)}>First-view help</button></div>
    {inspect ? <pre className="ev-state">{JSON.stringify({layout:names[variant],entry,draft,saved,active,editing,selected,matchingIds:results.map(m=>m.id)},null,2)}</pre> : null}
