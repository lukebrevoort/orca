...
describe("App", () => {
  ...
  test("renders the Gmail reconnect action in the top sync alert", () => {
    ...
    expect(html).toContain('class="inbox-sync-alert"');
    expect(html).toContain('role="alert"');
    expect(html).toContain('href="/settings/integrations/gmail"');
  ...
  test("keeps stacked Gmail actions on existing account-aware routes", () => {
    ...
    expect(buildGmailResyncRequest("gmail_work")).toEqual({ path: "/v1/sync/gmail/reset?accountId=gmail_work", method: "POST" });
  ...
  test("renders remote email images inert with an accessible per-message opt-in", () => {
    const html = renderToStaticMarkup(
      <RemoteImageBody
    ...
    expect(html).toContain("Remote images are hidden to protect your privacy.");
    expect(html).toContain('aria-pressed="false"');
    expect(html).toContain(">Load remote images</button>");
    expect(html).toContain('data-orca-remote-src="https://tracker.example/pixel.gif"');
    expect(html).not.toMatch(/<img[^>]+\ssrc="https:\/\/tracker\.example\/pixel\.gif"/);
    expect(html).toContain('src="cid:inline-logo"');
  });
...
describe("message subject containment", () => {
  test("keeps a long subject in the native title while truncating only its visual box", () => {
    const subject = "Invitation: Luke flight — United UA 2274: SFO → DEN @ Sun Aug 16, 2026 1:43pm - 4:32pm (EDT) (Luke Brevoort)";
