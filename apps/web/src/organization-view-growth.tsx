...
function GrowthChooser<T>(props: Props<T>) {
  ...
  if (review) return <OrganizationViewAuthoringWorkspace {...props} entry={review} onCancel={() => { setReview(null); setRetry(value => value + 1); }}/>;
  ...
  return <section className="views-workspace view-growth-chooser" aria-labelledby="views-title">
    ...
    {blocker ? <p className="view-state" role="status">{blocker}</p> : null}
