...
const trace: OrcaEvaluationTrace = {
  ...
  observedValues: [{ field: "sender.domain", present: true, value: "vercel.com" }, { field: "subject", present: true, value: "Production deploy failed" }],
  predicateResults: [{ revisionId: "rule-production-r2", predicate: "when:2", kind: "all", result: true, observedFields: [] }],
  candidates: [
  ...
  winners: [{ candidateId: "rule:lane", action: { kind: "route_lane", laneId: "lane-focus" }, slot: "lane", precedence: "rule_revision", ruleOrder: 0, actionOrder: 0, actor: { id: "system:gmail-sync", type: "system" }, reason: "A failed deploy blocks work", authorized: true, revisionId: "rule-production-r2" }],
  losers: [{ candidateId: "fallback:lane", action: { kind: "route_lane", laneId: "lane-fallback" }, slot: "lane", precedence: "workspace_fallback", ruleOrder: 0, actionOrder: 0, actor: { id: "system:workspace-fallback", type: "system" }, candidateReason: "Workspace fallback", reason: "higher_precedence_candidate", authorized: true, winnerCandidateId: "rule:lane" }],
...
beforeEach(() => {
  ...
  setGlobal("fetch", async (input: RequestInfo | URL) => {
    if (String(input) === "/v1/organization/evaluations/latest") return Response.json({ trace });
...
describe("Organization Glass Box Trace", () => {
  test("explains a live production-failure evaluation and opens its complete deterministic Trace", async () => {
    ...
    expect(browser.document.body.textContent).toContain("rule-production-r2");
    expect(browser.document.body.textContent).toContain("Workspace fallback");
    expect(browser.document.body.textContent).toContain("system:gmail-sync");
