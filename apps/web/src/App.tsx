...
function RemindMeControl({ threadId, reminder, onSave, onFinish }: { threadId: string; reminder: Reminder | null; onSave: (input: { threadId: string; scheduledFor: string; timezone: string; notify: boolean }) => Promise<void>; onFinish: (reminder: Reminder, cancelled?: boolean) => Promise<void> }) {
  ...
  return <div className="remind-control"><button aria-expanded={expanded} onClick={() => setExpanded((current) => !current)} type="button">↻ Remind me</button>{expanded ? <div className="remind-menu"><button onClick={() => preset(1)} disabled={saving} type="button">Tomorrow</button><button onClick={() => preset(7)} disabled={saving} type="button">Next week</button><label>Custom date and time<input aria-label="Custom reminder time" type="datetime-local" value={when} onChange={(event) => setWhen(event.target.value)} /></label><label className="remind-notify"><input checked={notify} onChange={(event) => setNotify(event.target.checked)} type="checkbox" /> Notify me when it returns</label><button disabled={!when || saving} onClick={() => { const date = new Date(when); if (!Number.isNaN(date.getTime())) void save(date); }} type="button">{saving ? "Saving…" : "Schedule"}</button><small>Times use {timezone}.</small></div> : null}</div>;
}
