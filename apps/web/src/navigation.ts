...
export function createSidebarNavigationProjection({ account, active, attention = false, collections, destinations = [], views = [], counts, draftCount, hidden, inboxCount, known = true, labels, online, order, syncing = false }: {
  ...
  return {
    ...
    online,
    spaces: [...destinations.filter(item => !item.retiredAt && !item.isFallback).sort((a,b) => a.position-b.position).map(item => ({ id: item.id, label: item.name, color: item.color, description: "mail space", kind: "destination" as const, count: item.counts.total })), ...projectWorkflowSpaces({ collections, views, counts, hidden, labels, order })],
  };
}
