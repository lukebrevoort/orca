...
export function ManageSpacesDialog({ busy = false, error = null, spaces, savedViews = [], demoMode = false, onDeleted, onClose, onCreate, onHide, onOpen, onManageViews, onEditView, onReorder, onRename, onRestore }: {
  ...
  const [pendingDelete, setPendingDelete] = useState<WorkflowSpace | null>(null);
  const [deleting, setDeleting] = useState(false);
  const deleteButtons = useRef(new Map<string, HTMLButtonElement>());
  const closeButton = useRef<HTMLButtonElement>(null);
  const focusAfterDelete = useRef<string | null | undefined>(undefined);
  function cancelDelete() {
    focusAfterDelete.current = pendingDelete?.id;
    setPendingDelete(null);
  }
  const close = () => { if (deleting) return; if (pendingDelete) cancelDelete(); else onClose(); };
  useEffect(() => {
    if (pendingDelete || focusAfterDelete.current === undefined) return;
    (focusAfterDelete.current ? deleteButtons.current.get(focusAfterDelete.current) : null)?.focus();
    if (!focusAfterDelete.current || !deleteButtons.current.has(focusAfterDelete.current)) closeButton.current?.focus();
