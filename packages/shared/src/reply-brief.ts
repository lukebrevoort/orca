...
const replyBriefItemSchema = z.object({
...
export const replyBriefOutputSchema = z.object({
  ...
  const referencedItems = [value.intent, ...value.facts, ...value.constraints, ...value.questions, ...value.considerations];
...
export function createDeterministicReplyBrief(input: unknown): ReplyBriefOutput {
  ...
  return replyBriefOutputSchema.parse({
    ...
    constraints: duration ? [{ text: `The request specifies a ${duration.toLowerCase()} duration.`, sourceRefs: [selectedMessage.sourceRef] }] : [],
    questions: schedulingRequest
      ? [{ text: "Which available time should the user choose?", sourceRefs: [selectedMessage.sourceRef] }]
      : [{ text: "What outcome does the user want from this response?", sourceRefs: [selectedMessage.sourceRef] }],
    considerations: context.availability
      ? [{ text: "Compare the proposed times only with the supplied free/busy intervals; no calendar event details were shared.", sourceRefs: [selectedMessage.sourceRef, "availability:free-busy"] }]
      : [{ text: "Calendar availability was not requested or shared.", sourceRefs: [selectedMessage.sourceRef] }],
    sourceRefs,
    confidence: {
