...
process.env.SESSION_SECRET = "bre385-disposable-local-review-session-secret";
process.env.TOKEN_ENCRYPTION_KEY = Buffer.alloc(32, 38).toString("base64");
const directory = mkdtempSync(join(tmpdir(), "orca-bre385-review-"));
...
async function requestReview(pathname: string, body: unknown) {
  const response = await app.request(pathname, { method: "POST", headers: { cookie: `orca_session=${session.token}`, "content-type": "application/json" }, body: JSON.stringify(body) });
  if (!response.ok) throw new Error(`Seed ${pathname} failed: ${response.status} ${await response.text()}`);
  return response.json() as Promise<any>;
}
app.get("/v1/review/login", (c) => { c.header("set-cookie", `orca_session=${session.token}; Path=/; HttpOnly; SameSite=Lax`); return c.redirect(saved.navigation.href); });
...
const prepared = await requestReview("/v1/organization/views/prepare", { kind: "typed_definition", source: { kind: "manual", label: "Reviewer saved View" }, identity: { name: "Project conversations", description: "Disposable stored mail for corrections and saved lifecycle review", color: "#70867d", position: 0 }, definition: { revision: 1, accountIds: ["work"], thread: { subjectContains: "Project" }, humanSignal: { minimumScore: 7 }, sender: { domains: ["example.com"] } }, unsupportedClauses: [] });
const { mode, viewId, viewRevision, source, identity, definition, unsupportedClauses } = prepared.draft;
const preview = await requestReview("/v1/organization/views/preview", { draft: { mode, viewId, viewRevision, source, identity, definition, unsupportedClauses }, page: { limit: 5 } });
const saved = await requestReview("/v1/organization/views/commit", { draft: preview.draft, expectedRevisions: { workspace: preview.workspaceRevision, view: null }, retryKey: "bre385-review-seed", confirmedZeroMatchDigest: null });

let failNextCommit = false;
const api = Bun.serve({ hostname: "127.0.0.1", port: 3085, fetch: (request) => {
...
const web = await createServer({ configFile: false, root: resolve("apps/web"), plugins: [react()], server: { host: "127.0.0.1", port: 5185, strictPort: true, proxy: { "/v1": "http://127.0.0.1:3085", "/health": "http://127.0.0.1:3085" } } });
await web.listen();
console.log(`BRE-385 scratch database: ${path}\nOpen http://127.0.0.1:5185/v1/review/login\nAdd future mail: curl -X POST http://127.0.0.1:5185/v1/review/future\nStop with Ctrl-C. The scratch database remains available for inspection.`);
