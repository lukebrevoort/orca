...
if (process.argv.includes("--built")) {
  ...
  const receipt = { mode: "built-SPA", harnessFixtureControls: { providerRefreshNoop, previewHoldTimeoutMs }, productAncestorSha: "0e7762cf1977f77151a422963f73e9a10188b866", runtimeSourceSha: "ce1bc03a0e34ed5dc9446656108f5f202787054e", harnessCheckoutSha: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), harnessSha256: createHash("sha256").update(readFileSync(import.meta.path)).digest("hex"), buildReceipt: "docs/verification/guidance-contrast/final-build.json", buildReceiptSha256: createHash("sha256").update(readFileSync("docs/verification/guidance-contrast/final-build.json")).digest("hex"), assetTreeSha256: createHash("sha256").update(JSON.stringify(assets)).digest("hex"), assets };
