...
if (process.argv.includes("--built")) {
  ...
  const receipt = { mode: "built-SPA", harnessFixtureControls: { providerRefreshNoop, previewHoldTimeoutMs }, productAncestorSha: "0e7762cf1977f77151a422963f73e9a10188b866", runtimeSourceSha: "465be988438e12631a4e201f6706a6f8e0dfb035", harnessCheckoutSha: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), harnessSha256: createHash("sha256").update(readFileSync(import.meta.path)).digest("hex"), buildReceipt: "docs/verification/preview-error-layout/fixed-build.json", buildReceiptSha256: createHash("sha256").update(readFileSync("docs/verification/preview-error-layout/fixed-build.json")).digest("hex"), assetTreeSha256: createHash("sha256").update(JSON.stringify(assets)).digest("hex"), assets };
