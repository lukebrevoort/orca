...
if (process.argv.includes("--built")) {
  ...
  const receipt = { mode: "built-SPA", harnessFixtureControls: { providerRefreshNoop, previewHoldTimeoutMs }, productAncestorSha: "0e7762cf1977f77151a422963f73e9a10188b866", runtimeSourceSha: "d0b05f8d20992e76276283c63c843119291b6bb6", harnessCheckoutSha: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), harnessSha256: createHash("sha256").update(readFileSync(import.meta.path)).digest("hex"), buildReceipt: "docs/verification/summary-focus/final-provenance.json", buildReceiptSha256: createHash("sha256").update(readFileSync("docs/verification/summary-focus/final-provenance.json")).digest("hex"), assetTreeSha256: createHash("sha256").update(JSON.stringify(assets)).digest("hex"), assets };
